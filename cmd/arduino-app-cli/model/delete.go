...
func modelDeleteHandler(ctx context.Context, cfg config.Configuration, id string, force bool) {
	err := orchestrator.AIModelDelete(ctx, servicelocator.GetDockerClient(), cfg, servicelocator.GetModelsIndex(), servicelocator.GetModelsHandlerIndex(), servicelocator.GetBricksIndex(), servicelocator.GetPlatform(), id, servicelocator.GetAppIDProvider(), force)
	if err != nil {
