...
```python
class WebUI(addr: str, port: int, ui_path_prefix: str, api_path_prefix: str, assets_dir_path: str, certs_dir_path: str, use_ssl: bool)
```
...
- **certs_dir_path** (*str*) (optional), default="/app/certs": Path to SSL certificates directory. Defaults to "/app/certs".
- **use_ssl** (*bool*) (optional), default=False: Enable SSL/HTTPS. Defaults to False.
...
- **RuntimeWarning**: If the server is not currently running.

#### `WebUI.expose_api(method: str, path: str, function: callable)`
...
#### `WebUI.on_connect(callback: Callable[[str], None])`
...
#### `WebUI.on_disconnect(callback: Callable[[str], None])`
...
#### `WebUI.on_message(message_type: str, callback: Callable[[str, any], any])`
...
- **callback** (*Callable[[str, any], any]*): Function to handle the message. Receives two arguments: the session ID (sid) and the incoming message data.

#### `WebUI.send_message(message_type: str, message: dict | str, room: str)`
