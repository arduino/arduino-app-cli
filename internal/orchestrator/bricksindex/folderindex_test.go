...
func TestLoadBrickFromFolder(t *testing.T) {
	folderIndex := loadFromFolder(paths.New("testdata/bricks"))
	...
	assert.Equal(t, "testdata/bricks/my-first-brick/brick_compose.yaml", folderIndex[0].composeFile.String())
	assert.NotNil(t, folderIndex[0].readmeFile)
	assert.Equal(t, "testdata/bricks/my-first-brick/README.md", folderIndex[0].readmeFile.String())
	assert.NotNil(t, folderIndex[0].examplesPath)
	assert.Equal(t, "testdata/bricks/my-first-brick/examples", folderIndex[0].examplesPath.String())
}
