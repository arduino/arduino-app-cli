...
- id: arduino:another-brick-with-camera-device
  ...
  - camera`)
	...
	t.Run("fail if a camera device is not detected and one of two brick require a physical camera", func(t *testing.T) {
		...
		err = generateMainComposeFile(&a, bricksIndex, "app-bricks:python-apps-base:dev-latest", cfg, map[string]string{}, store.NewStaticStore(cfg.AssetsDir().String()), &availableDevices)
		require.Error(t, err)
	})
