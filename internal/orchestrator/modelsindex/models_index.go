...
type Iterable[T any] func() iter.Seq[T]

type ModelsIndex struct {
	modelIterable []Iterable[AIModel]
}
...
func (m *ModelsIndex) getIter() iter.Seq[AIModel] {
	return func(yield func(AIModel) bool) {
		for _, iterable := range m.modelIterable {
			iterable()(yield)
		}
	}
}
...
func Load(dir *paths.Path, customModelDir *paths.Path) (*ModelsIndex, error) {
	...
	return &ModelsIndex{modelIterable: []Iterable[AIModel]{
		...
		func() iter.Seq[AIModel] { return LoadEdgeImpulseModels(edgeimpulseModelsDir) },
	},
	}, nil
}
