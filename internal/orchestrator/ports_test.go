...
func TestAppPorts(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "the ports declared by the app.yaml",
			appPorts: []int{7000},
			want:     []PortInfo{{Port: "7000", Source: "app.yaml"}},
		},
		{
			name:   "every port of a brick reports the display the brick requires",
			bricks: []app.Brick{{ID: "arduino:dashboard"}},
			want: []PortInfo{
				{Port: "7100", Source: "arduino:dashboard", RequiresDisplay: "webview"},
				{Port: "7101", Source: "arduino:dashboard", RequiresDisplay: "webview"},
			},
		},
		{
			name:   "a brick requiring no display reports none",
			bricks: []app.Brick{{ID: "arduino:data_logger"}},
			want:   []PortInfo{{Port: "8080", Source: "arduino:data_logger"}},
		},
		{
			name:   "the ports of a required service are reported against the service",
			bricks: []app.Brick{{ID: "arduino:asr"}},
			want:   []PortInfo{{Port: "8085", Source: "arduino:audio"}},
		},
		{
