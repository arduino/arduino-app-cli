...
func TestAppPorts(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			appPorts: []int{7000},
			want:     []PortInfo{{Port: "7000", Source: "app.yaml", Intent: UserIntent}},
		},
		{
			name:   "every port of a brick requiring a display is a webview one",
			bricks: []app.Brick{{ID: "arduino:dashboard"}},
			want: []PortInfo{
				{Port: "7100", Source: "arduino:dashboard", Intent: WebviewIntent, RequiresDisplay: "webview"},
				{Port: "7101", Source: "arduino:dashboard", Intent: WebviewIntent, RequiresDisplay: "webview"},
			},
		},
		{
			name:   "the ports of a brick requiring no display are for the user",
			bricks: []app.Brick{{ID: "arduino:data_logger"}},
			want:   []PortInfo{{Port: "8080", Source: "arduino:data_logger", Intent: UserIntent}},
		},
		{
			name:   "the ports of a required service are internal and reported against the service",
			bricks: []app.Brick{{ID: "arduino:asr"}},
			want:   []PortInfo{{Port: "8085", Source: "arduino:audio", Intent: InternalIntent}},
		},
		{
		...
		},
		{
			...
			want: []PortInfo{
				{Port: "6000", Source: "app.yaml", Intent: UserIntent},
				{Port: "7000", Source: "arduino:web_ui", Intent: WebviewIntent, RequiresDisplay: "webview"},
				{Port: "9999", Source: "arduino:db", Intent: InternalIntent},
			},
		},
	}
