...
```python
class KeywordSpotter(mic: Microphone, confidence: float, debounce_sec: float)
```

KeywordSpotter module for classifying audio data to detect keywords using a specified model.
...
#### `KeywordSpotter.on_detect(keyword: str, callback: Callable[[], None])`
